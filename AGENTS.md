...
> **Current release:** v0.2.2. **Main current:** post-release checkpoint adds locked-Hard casualty-tolerance integration, #7 role-aware campaign perk steering, and a hardened #22 W&L command-selection retry on top of v0.2.2, built and deployed (`dist`/BepInEx SHA-256 `b5f86e5cdb4ef669a25ca6adfb406da343b100344345aa423d2df0e02b31075e`). Current main includes fiscal economy AI, recruitment state steering, policy/naval grand-strategy timing, default-on fast-forward catch-up, hot-path reflection caching for startup/menu lag, W&L command-selection prompt retry, historical-Hard casualty tolerance routed into `CIC.Effective`, and campaign army/fleet perk scoring. v0.2.2 runtime smoke is confirmed; the post-release additions need in-game first-fire smoke. `FormationOffensiveSafetyPatch` remains intentionally deferred until the directive ledger has more runtime soak.
