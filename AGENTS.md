...
> **Current release:** v0.2.2. **Main current:** post-release checkpoint `a0cf709` adds locked-Hard casualty-tolerance integration, #7 role-aware campaign perk steering, and hardened #22 W&L command-selection retry on top of v0.2.2, built and deployed (`dist`/BepInEx SHA-256 `3b0423317bd62c78062bf62929dee6e6432dbda029e865196b82c9eaa88c630d`). Current main includes fiscal economy AI, recruitment state steering, policy/naval grand-strategy timing, default-on fast-forward catch-up, hot-path reflection caching for startup/menu lag, W&L command-selection prompt retry, historical-Hard casualty tolerance routed into `CIC.Effective`, and campaign army/fleet perk scoring. Runtime smoke confirmed the W&L picker works, time advances after command selection, and normal campaign systems fire without #22 errors; actual #7 perk assignment remains conditional on vanilla perk timing. `FormationOffensiveSafetyPatch` remains intentionally deferred until the directive ledger has more runtime soak.
