...
- Bug Fixes now has a durable workstream at `docs/bug-fixes/`. The first seeded queue is `docs/bug-fixes/vanilla-ai-economy.md`: `BUG-ECO-001` has regression coverage for the runtime-confirmed fiscal subsidy sentinel leak; `BUG-ECO-003` has #28 guard code for the repeated `Economy.UpdateEconomyAllianceData` Player.log NRE. DLL build/deploy/hash is verified for `ce17af99fe147e3ee4c2d16884522b19714a5c2960a4262782a86ca680acaeb0`; fresh-launch smoke is still required before marking them shipped. Policy null-personality, supply-depot construction, and railroad construction remain repro/backlog items.
