...
- Bug Fixes now has a durable workstream at `docs/bug-fixes/`. The first seeded queue is `docs/bug-fixes/vanilla-ai-economy.md`: `BUG-ECO-001` has local regression coverage for the runtime-confirmed fiscal subsidy sentinel leak; `BUG-ECO-003` has #28 local guard code for the repeated `Economy.UpdateEconomyAllianceData` Player.log NRE. Both still need build/deploy/hash and fresh-launch smoke before being marked shipped. Policy null-personality, supply-depot construction, and railroad construction remain repro/backlog items.
