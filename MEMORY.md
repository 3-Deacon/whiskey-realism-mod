...
- Bug Fixes now has a durable workstream at `docs/bug-fixes/`. The seeded queues are `docs/bug-fixes/vanilla-ai-economy.md` and `docs/bug-fixes/vanilla-tick-system.md`: `BUG-ECO-001` has regression coverage for the runtime-confirmed fiscal subsidy sentinel leak; `BUG-ECO-003` has #28 guard code for the repeated `Economy.UpdateEconomyAllianceData` Player.log NRE; `BUG-TICK-001` extends #26 with a paused `AICampaign.Update` guard for vanilla's `Mathf.Max(1, sqrt(GameVars.gamespeed))` speed-zero tick. Current deployed bug-fix DLL SHA-256 is `4e01274babb3af3def1143ab4acee6db8a31c27817d80c1789385b60e3c0f19f`; fresh-launch smoke is still required before marking them shipped. Policy null-personality, supply-depot construction, railroad construction, and the unbounded `CampaignDataRuns` filter-map initialization loop remain repro/backlog items.
