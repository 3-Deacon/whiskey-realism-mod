...
Tactical behavior valves are intentionally conservative. B3 odds telemetry is read-only, but B4 writes battle-level `macroai` and B5 writes group `ai_stance`; both stay default-off until focused in-game smoke proves bounded `[TacticalMacroDecision]` / `[TacticalGroupDecision]` logs, stable Harmony anchors, no repeated exceptions, no charge stance 4 writes, and no movement/reserve/artillery/fallback side effects.
