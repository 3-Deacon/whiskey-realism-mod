...
**v0.2.2 released. Post-release `project-doctrine` is built and deployed locally with SHA-256 `cc9fdfe62e9000e35ced392a02cc439d34ec3d5e7d5ca1cddfe10e00cb0b5e0b` (588800 bytes). Runtime smoke confirms the W&L command picker works, time advances after command selection, normal campaign systems fire without #22 errors, project-doctrine selection fires, and the Slice B0/B2 tactical observer surfaces are readable in W&L battles. Current tactical build adds removable `[TacticalDecisionMatrix]` battle/group rows for AI decision inspection and #46 `BattleObjectiveChainWlGuardPatch`, which blocks W&L objective-chain advance for AI center groups carrying attached player-subordinate units behind `Enable W&L Tactical Charge Guard`. Live matrix smoke found and fixed no-measured-enemy weak-point inflation, a matrix-only W&L/order-friction label inversion, and B4 macro scoring that used force-balance/no-sector inference instead of actual `AIBattle.unitsused` contact/sector evidence. The follow-up B5 probe fix now maps defensive weak points to vanilla screening/probe stance 1 and lets explicit Probe sectors bypass the earlier low-confidence skip, so vanilla can form probing groups into line instead of leaving them held in column/march-column behavior; the retreat follow-up preserves vanilla ownership when `macroai == 3` so B5 cannot probe during retreat. No-contact odds now report zero current/projected odds instead of fake superiority. Construction steering Slice B is deployed; optional telegraph AI remains default-off and still needs an enabled smoke run. The local `project-doctrine` branch also includes W&L dispatch/current-order bridging, coordinated operation packages, default-on historical operation doctrine, Slice B1 W&L charge/feud guards, Slice B2 read-only command/order-friction telemetry, B3 `[TacticalOdds]` doctrine telemetry, default-off #43 fallback/retreat NRE containment, and default-off-by-default B4/B5 tactical macro/group stance behavior. Fresh runtime smoke is still needed for `[HistoricalOperation]`, `[CoordinatedOps]`, `[TacticalObjectiveGuard]`, B5 probe/line-formation behavior, and `[Patch:TacticalFallbackRetreatNullGuard]` if those paths exercise.**
...
Tactical behavior valves are intentionally conservative. B3 odds telemetry is read-only, but B4 writes battle-level `macroai` and B5 writes group `ai_stance`; both stay default-off until focused in-game smoke proves bounded `[TacticalMacroDecision]` / `[TacticalGroupDecision]` logs, visible probe/screen stance behavior without charge stance 4 writes, stable Harmony anchors, no repeated exceptions, and no movement/reserve/artillery/fallback side effects.
