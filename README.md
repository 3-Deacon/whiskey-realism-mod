...
**v0.2.2 released. Current development state is tracked in living docs, not this README.** Post-release tactical work now includes tactical orchestrator O0/O1/O2/O3, #58 deployment observer, Slice 0 command-node tree, Slice 1 reserve commitment gate, Slice 3 #41 command-role charge gate, #60 deployment terrain/facing discipline, #61 tactical operations-ledger posture execution, full-spectrum doctrine consumers for stance/charge/reserve/fallback/artillery, and #62 default-off W&L player-order doctrine. The current player-order worktree is build/deploy/hash verified with console harness **898 PASS / 0 FAIL**, `./build.sh` clean, and deployed DLL SHA-256 `5398831614e855f90b156c734fee50348d0bb6b10c8f0390b7a3d2499ba57740` (977920 bytes). Fresh Active command-doctrine smoke and #62 W&L player-order diagnostics/write smoke are still pending. See [`docs/handoff.md`](docs/handoff.md), [`docs/tactical-orchestrator.md`](docs/tactical-orchestrator.md), [`docs/tactical-operations-ledger.md`](docs/tactical-operations-ledger.md), [`docs/tactical-terrain-facing-discipline.md`](docs/tactical-terrain-facing-discipline.md), and [`docs/wl-player-order-doctrine.md`](docs/wl-player-order-doctrine.md).
...
Tactical behavior valves are intentionally conservative. `Tactical Commander Mode = Active` is the release/default mode for the operations-ledger command system; `MonitorOnly` runs ledger and diagnostics without writes, and `Off` is rollback. Current doctrine consumers keep one owner per vanilla surface: #45 stance, #41 charge, B8 reserve/fallback, B7 artillery, and #61 posture execution read the same command doctrine order. #62 player-facing W&L current orders have a separate write valve: diagnostics are default-on, but `Enable W&L Player Order Doctrine` is default-off until focused smoke proves bounded behavior. Older tactical valves remain separate where not subsumed: B3 odds telemetry and #58 deployment telemetry are read-only, while B4 can write battle-level `macroai`, #53 can change private `Regiment.AddPath` outcomes, and #60 can correct AI deployment terrain/deployment-zone failures through vanilla `SetGroupFormation`. For any tactical writer, fresh in-game smoke must prove bounded logs, stable Harmony anchors, no repeated exceptions, no unintended player-chain retasking outside the named W&L player-order surface, and no unintended movement/reserve/artillery/fallback/charge/path side effects.
