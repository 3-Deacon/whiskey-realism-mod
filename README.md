...
**v0.2.2 released. Current development state is tracked in living docs, not this README.** Post-release tactical work now includes tactical orchestrator O0/O1/O2/O3, #58 deployment observer, Slice 0 command-node tree, Slice 1 reserve commitment gate, Slice 3 #41 command-role charge gate, #60 deployment terrain/facing discipline, and #61 tactical operations-ledger posture execution. Current `main` is build/deploy/hash verified with console harness **763 PASS / 0 FAIL**, `./build.sh` clean, and deployed DLL SHA-256 `a1f7c1e575fe984477d233c63a98b194c719ee6b28b9172d960661d8ca3944ca` (888832 bytes). Fresh Active operations-ledger smoke is still pending because the current log predates this deploy. See [`docs/handoff.md`](docs/handoff.md), [`docs/tactical-orchestrator.md`](docs/tactical-orchestrator.md), [`docs/tactical-operations-ledger.md`](docs/tactical-operations-ledger.md), and [`docs/tactical-terrain-facing-discipline.md`](docs/tactical-terrain-facing-discipline.md).
...
Tactical behavior valves are intentionally conservative. `Tactical Commander Mode = Active` is the release/default mode for the operations-ledger command system; `MonitorOnly` runs ledger and diagnostics without writes, and `Off` is rollback. Older tactical valves remain separate: B3 odds telemetry and #58 deployment telemetry are read-only, while B4 writes battle-level `macroai`, B5 writes group `ai_stance`, B7 can change artillery combat behavior, B8 can call vanilla `BattleUnits.SetWithdrawal`, #53 can change private `Regiment.AddPath` outcomes, and #60 can correct AI deployment terrain/deployment-zone failures through vanilla `SetGroupFormation`. For any tactical writer, fresh in-game smoke must prove bounded logs, stable Harmony anchors, no repeated exceptions, no player-subordinate retasking, and no unintended movement/reserve/artillery/fallback/charge/path side effects.
