...
**v0.2.2 released. Post-release tactical work is built and deployed locally with SHA-256 `b07bbd39eaaf664d81d5930b42d5dc268b64c43dc98244a17015c49e329ce88a` (637952 bytes) and console harness **503 PASS / 0 FAIL**. Runtime smoke confirms the W&L command picker works, time advances after command selection, normal campaign systems fire without #22 errors, project-doctrine selection fires, and the Slice B0/B2 tactical observer surfaces are readable in W&L battles. Current tactical work includes removable `[TacticalDecisionMatrix]` battle/group rows, #46 `BattleObjectiveChainWlGuardPatch`, B4/B5 default-off tactical macro/group stance scoring, B7+B8 artillery/withdrawal runtime wiring, and #53 default-off `TacticalPathfinderDisciplinePatch` for BUG-TAC-010. Focused B7+B8 smoke with the live config intentionally setting `Enable Tactical Artillery Doctrine = true` and `Enable Tactical Withdrawal Doctrine = true` confirmed `[once:b7-check-ai-bombardment]`, `[once:b7-counterbattery]`, `[once:b8-check-line-fallbacks]`, `[once:b8-morale-snapshot-sampler]`, `[once:b8-check-reserves]`, and `[once:b8-set-withdrawal]`, with no B7/B8 repeated exceptions, Harmony failures, missing-anchor warnings, or writer error markers. `[once:b8-microai-check-retreats]`, conditional `[once:b7-cancel-bombard]`, and #53 enabled `[TacticalPathfinderDiscipline]` smoke still need observation. C# defaults keep B7/B8 writer valves and the pathfinder discipline valve false; the current live config has the B7/B8 and pathfinder valves enabled only as a focused smoke override, not a default-on policy. Fresh runtime smoke is still needed for `[HistoricalOperation]`, `[CoordinatedOps]`, `[TacticalObjectiveGuard]`, B5 probe/line-formation behavior, `[Patch:TacticalFallbackRetreatNullGuard]`, and the pending tactical conditional markers if those paths exercise.**
