...
**v0.2.2 released. Post-release tactical work is active. Current `main` is hash-deployed locally through tactical orchestrator Slice 1, while the `tactical-terrain-facing-discipline` worktree adds #60 `TacticalDeploymentTerrainDisciplinePatch` plus #58 terrain/facing telemetry. The feature worktree DLL and deployed BepInEx plugin currently match SHA-256 `0b9a5d2274d5c79ebfa64a9cfa36ffc21a3e3100ffdbfb3ed68a5810819f853c` (834048 bytes). The live local config has `Enable Tactical Deployment Terrain Discipline = true` for focused smoke, but runtime smoke is still pending. Current tactical living docs are [`docs/tactical-terrain-facing-discipline.md`](docs/tactical-terrain-facing-discipline.md), [`docs/bug-fixes/vanilla-tactical-battlefield.md`](docs/bug-fixes/vanilla-tactical-battlefield.md), [`docs/handoff.md`](docs/handoff.md), and [`docs/patch-catalog.md`](docs/patch-catalog.md).**
...
Tactical behavior valves are intentionally conservative. B3 odds telemetry and #58 deployment telemetry are read-only, but B4 writes battle-level `macroai`, B5 writes group `ai_stance`, B7 can change artillery combat behavior, B8 can call vanilla `BattleUnits.SetWithdrawal`, #53 can change private `Regiment.AddPath` outcomes, and #60 can correct AI deployment terrain/deployment-zone failures through vanilla `SetGroupFormation`. Their C# defaults stay false until focused in-game smoke proves bounded logs, stable Harmony anchors, no repeated exceptions, no player-subordinate retasking, and no unintended movement/reserve/artillery/fallback/charge/path side effects. The live smoke config currently has several behavior valves enabled, including #60, only for focused local testing.
...
- [`docs/superpowers/specs/2026-05-10-tactical-terrain-facing-discipline-design.md`](docs/superpowers/specs/2026-05-10-tactical-terrain-facing-discipline-design.md)
- [`docs/superpowers/plans/2026-05-10-tactical-terrain-facing-discipline-implementation-plan.md`](docs/superpowers/plans/2026-05-10-tactical-terrain-facing-discipline-implementation-plan.md)
