...
**v0.2.2 released. Current development state is tracked in living docs, not this README.** Post-release `main` now includes the full tactical-orchestrator/operations-ledger workstream plus the structured telemetry framework. Tactical development includes O0/O1/O2/O3, #58 deployment observer, Slice 0 command-node tree, Slice 1 reserve commitment gate, Slice 3 #41 command-role charge gate, #60 deployment terrain/facing discipline, #61 tactical operations-ledger posture execution, #62 default-off W&L player-subordinate order bridge, full-spectrum doctrine consumers for stance/charge/reserve/fallback/artillery/player-order intent, true frontage/echelon/artillery-line battle geometry, route-corridor path-quality scoring, reserve/fallback/artillery/endurance gates, Scourge-style play/courier/cavalry/fire-control conversions, and fog-safe runtime objective anchoring from vanilla objective chains, current objectives, live scene `Objectives`, or movement waypoints. Telemetry defaults to `[Telemetry] Logging Profile = Off`; tuning profiles write JSONL sidecars under `<GTCW>/BepInEx/WhiskeyRealism/tuning-logs/<session-id>/` instead of using `LogOutput.log` as the normal firehose. Current `main` is build/deploy/hash verified with console harness **1075 PASS / 0 FAIL**, `./build.sh` clean, and deployed DLL SHA-256 `cfdb9018bc0cb7c0fcb7ba1e28acac0b1b119243856ef3a027716f8b9b930e75` (1245184 bytes). Fresh Active command-doctrine smoke and telemetry profile smoke are still pending because the current log mtime `2026-05-15 07:54:28 -0500` predates the deployed plugin timestamp `2026-05-15 20:20:19 -0500`, and no `tuning-logs` directory exists yet. See [`docs/handoff.md`](docs/handoff.md), [`docs/telemetry.md`](docs/telemetry.md), [`docs/tactical-orchestrator.md`](docs/tactical-orchestrator.md), [`docs/tactical-operations-ledger.md`](docs/tactical-operations-ledger.md), and [`docs/tactical-terrain-facing-discipline.md`](docs/tactical-terrain-facing-discipline.md).
