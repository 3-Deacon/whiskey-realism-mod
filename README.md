...
**v0.2.2 released. Current development state is tracked in living docs, not this README.** Post-release `main` now includes the full tactical-orchestrator/operations-ledger workstream plus the structured telemetry framework. Tactical development includes O0/O1/O2/O3, #58 deployment observer, Slice 0 command-node tree, Slice 1 reserve commitment gate, Slice 3 #41 command-role charge gate, #60 deployment terrain/facing discipline, #61 tactical operations-ledger posture execution, #62 default-off W&L player-subordinate order bridge, #63 default-off tactical map-knowledge diagnostics, full-spectrum doctrine consumers for stance/charge/reserve/fallback/artillery/player-order intent, true frontage/echelon/artillery-line battle geometry, route-corridor path-quality scoring, reserve/fallback/artillery/endurance gates, Scourge-style play/courier/cavalry/fire-control conversions, and fog-safe runtime objective anchoring from vanilla objective chains, current objectives, live scene `Objectives`, or movement waypoints. Telemetry defaults to `[Telemetry] Logging Profile = Off`; tuning profiles write JSONL sidecars under `<GTCW>/BepInEx/WhiskeyRealism/tuning-logs/<session-id>/` instead of using `LogOutput.log` as the normal firehose. Current local build/deploy proof is console harness **1110 PASS**, `./build.sh` clean, and matching `dist`/deployed DLL SHA-256 `562a61b5cd0cbbedc6d6002a349cd3d68ebf50ea1d60c941e3a5a9deeaafc57a` (1327104 bytes). The freshest loaded runtime proof is from the prior `695be770...` TacticalTuning session: Active mode and tuning sidecars loaded, orders were issued, but the log exposed stale `groupsubordinatesmoving`/`activeMove` movement detection that could leave objective movement pulsing without sustained progress. The `562a61b5...` build fixes that stale movement-state classification and still needs a fresh game restart/smoke before claiming the runtime symptom is fixed. See [`docs/handoff.md`](docs/handoff.md), [`docs/telemetry.md`](docs/telemetry.md), [`docs/tactical-orchestrator.md`](docs/tactical-orchestrator.md), [`docs/tactical-operations-ledger.md`](docs/tactical-operations-ledger.md), and [`docs/tactical-terrain-facing-discipline.md`](docs/tactical-terrain-facing-discipline.md).
...
- [`docs/wl-player-order-doctrine.md`](docs/wl-player-order-doctrine.md)
- [`docs/tactical-brain.md`](docs/tactical-brain.md)
- [`docs/strategic-recon-commitment.md`](docs/strategic-recon-commitment.md)
