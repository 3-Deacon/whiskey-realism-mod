...
**v0.2.2 released. Post-release `project-doctrine` is built and deployed locally with SHA-256 `54c2adba6796e3fb7fbc826171bac2c7ff71029f8e092fd9fc765bc30892a214` (583680 bytes). Runtime smoke confirms the W&L command picker works, time advances after command selection, normal campaign systems fire without #22 errors, project-doctrine selection fires, and the Slice B0/B2 tactical observer surfaces are readable in W&L battles. Current tactical build adds removable `[TacticalDecisionMatrix]` battle/group rows for AI decision inspection and #46 `BattleObjectiveChainWlGuardPatch`, which blocks W&L objective-chain advance for AI center groups carrying attached player-subordinate units behind `Enable W&L Tactical Charge Guard`. The currently loaded old DLL already proved `[TacticalPathShape] reason=backward-first-segment`; restart is required before `[TacticalDecisionMatrix]` and `[TacticalObjectiveGuard]` can fire from the new DLL. Construction steering Slice B is deployed; optional telegraph AI remains default-off and still needs an enabled smoke run. The local `project-doctrine` branch also includes W&L dispatch/current-order bridging, coordinated operation packages, default-on historical operation doctrine, Slice B1 W&L charge/feud guards, Slice B2 read-only command/order-friction telemetry, B3 `[TacticalOdds]` doctrine telemetry, default-off #43 fallback/retreat NRE containment, and default-off B4/B5 tactical macro/group stance behavior. Fresh runtime smoke is still needed after restart for `[HistoricalOperation]`, `[CoordinatedOps]`, `[TacticalDecisionMatrix]`, `[TacticalObjectiveGuard]`, `[TacticalOdds]`, `[TacticalMacroDecision]`, `[TacticalGroupDecision]`, and `[Patch:TacticalFallbackRetreatNullGuard]` if those paths exercise.**
