...
| `BUG-TICK-003` | Shipped | W&L diary tick | Post-#37 Player.log showed two early `Diary.DMD<Diary::UpdateEvents>() -> Diary.UpdateEventQueue() -> BattleUnits.Update()` NREs. Vanilla `Diary.UpdateEventQueue` starts at frame 50, but `Diary.UpdateEvents` immediately dereferences W&L player command and imported diary dependencies without readiness guards. | #29 now skips only W&L `Diary.UpdateEvents` until the selected commander/current command and diary dependencies are ready, then falls through to vanilla. Build/deploy/hash verified in DLL `9d4c12fc...`; fresh-launch smoke confirmed on 2026-05-06. |
...
| `BUG-TAC-001` - `BUG-TAC-010` | Mixed | Tactical battlefield | Subagent-backed decompile review found courier queue, fallback/retreat null, W&L current-order, delayed waypoint, objective-chain, reserve, W&L incident-order-delay, and pathfinder-shape hazards. `BUG-TAC-005` objective-chain exposure now has #46 guard implemented; `BUG-TAC-010` pathfinder backtrack has live `[TacticalPathShape] reason=backward-first-segment` proof and #53 pathfinder discipline implemented. | Track in `vanilla-tactical-battlefield.md`; telemetry first except shipped #46 objective-chain guard, default-off #43 fallback/retreat NRE containment, and default-off #53 pathfinder discipline. |
