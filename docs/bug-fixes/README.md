...
| `BUG-TICK-002` | In progress | Economy tick | `BattleUnits.CampaignDataRuns` has an unbounded `while (!Economy.UpdateFilterMaps(initialization:true)) { }` loop at decompile line 79874. | #31 `EconomyFilterMapInitializationGuardPatch` bounds repeated no-progress initialization returns and initialization-only exceptions; build/deploy/hash verified in DLL `7da618bf...`; fresh-launch smoke pending. |
