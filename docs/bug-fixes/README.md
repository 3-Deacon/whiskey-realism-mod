...
| `BUG-TAC-001` - `BUG-TAC-009` | Mixed | Tactical battlefield | Subagent-backed decompile review found courier queue, fallback/retreat null, W&L current-order, delayed waypoint, objective-chain, reserve, and W&L incident-order-delay hazards. Focused smoke confirmed only `BUG-TAC-005` objective-chain player-subordinate exposure; path/position movement proof is still required. | Track in `vanilla-tactical-battlefield.md`; telemetry first except shipped default-off #43 fallback/retreat NRE containment. |
