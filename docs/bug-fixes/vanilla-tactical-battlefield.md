...
Focused W&L battle smoke ran with `Enable Tactical Observer = true`, `Enable Tactical Bug Telemetry = true`, `Enable Tactical Fallback Retreat Null Guard = true`, `Enable W&L Tactical Charge Guard = true`, verbose observer logging off, and summary throttle 30 seconds. That smoke ran on DLL SHA-256 `2a7bf702f6408e2a131a43fe6aa1a9ee04b56175ee292375c5167344c29c14c1` (519680 bytes). Current deployed DLL SHA-256 is `54c2adba6796e3fb7fbc826171bac2c7ff71029f8e092fd9fc765bc30892a214` (583680 bytes) and adds #46 objective-chain filtering plus config-gated `[TacticalDecisionMatrix]` rows. The currently loaded old DLL also produced `[TacticalPathShape] reason=backward-first-segment`; restart is required before `[TacticalDecisionMatrix]` and `[TacticalObjectiveGuard]` can appear from the new DLL. B4/B5 macro/group stance configs remain default-off because they write behavior-changing vanilla battle state and have not completed focused runtime smoke.
...
- `BUG-TAC-005` is guarded in code now because live exposure matched the vanilla center-only W&L gap and the player observed flank exposure in battle. Runtime denial proof is pending restart.
