...
| `BUG-TAC-013` | Implemented; enabled smoke pending | AI deployment terrain/facing | Vanilla `BattleUnits.DoPlacementAIUnitsWithinDeploymentzoneNew` places AI groups through immediate formation placement, then runs deployment-zone and water correction surfaces that can still leave in-bounds water/edge/odd placements. Vanilla final facing is caller/path-tail driven unless a manual rotation is supplied; closest-enemy fields are not sufficient Whiskey evidence without visible-enemy filtering. Anchors: `DoPlacementAIUnitsWithinDeploymentzoneNew` 85524-85872, vanilla cleanup 85873-85880, `SetGroupFormation` 91815/92056, water checks 131432, visible enemy lists 122545. | #58 now emits bounded `[TacDeployTerrain]` evidence. #60 `TacticalDeploymentTerrainDisciplinePatch` was verified in DLL `b00e03bd7e635e981380459e09a0d52a19d635c22c49bd340b403dacfbdf4cf8` (841216 bytes) and is included in the current deployed DLL `562a61b5cd0cbbedc6d6002a349cd3d68ebf50ea1d60c941e3a5a9deeaafc57a` (1327104 bytes; 1110 PASS), C# default-off behind `Enable Tactical Deployment Terrain Discipline`, and enabled in the live local config for focused smoke. It corrects only clear AI terrain/deployment-zone failures through vanilla `SetGroupFormation`, then mirrors vanilla cleanup for the corrected group tree. Living reference: `../tactical-terrain-facing-discipline.md`. |
| `BUG-TAC-015` | Patch shipped; smoke pending | CheckOutOfFireRange null deref on closestenemyunitfar | `AIBattle.CheckOutOfFireRange` (decompile lines 4497-4537) guards on `unitrange.closestenemyunitfarreg` (Regiment) at line 4518 but then dereferences `unitrange.closestenemyunitfar` (GameObject) at lines 4529 and 4532 without a null check. These are SEPARATE fields on `UnitRange` — line 109492 (GameObject) and 109496 (Regiment). Multiple vanilla call sites check the GameObject for null independently (36099, 36268, 93370, 93430, 116148, 122925), so the GameObject can be null while the Regiment ref is set (unit destruction mid-fallback). NRE in `CheckOutOfFireRange` kills the per-side microai tick. | Extended `TacticalFallbackRetreatNullGuardPatch` (shipped) to also Finalize `AIBattle.CheckOutOfFireRange` and added "CheckOutOfFireRange" to `ShouldSuppressFallbackRetreatException`. Gated by existing `Enable Tactical Fallback Retreat Null Guard` flag (default ON). |
| `BUG-TAC-014` | Guard implemented; fresh smoke pending | Stale interrupted command path/order state | Vanilla commander interruption stops command groups while preserving path state: `Regiment` checks `pathinterrupted || unitlinkedto`, `regimentpaths > 0`, `unittyp > 13`, sees a close enemy, then calls `StopRegiment(... clearregimentpathsbeforerotation:false ...)` at decompile lines 117179-117185. Vanilla clears/restores only after no close enemy remains at lines 117188-117205. The 2026-05-11 stale log shows many command groups in `formation=3:MarchColumn` with `pathInterrupted=True`, `queue=0`, `activeMove=False`, and often `orderState=1` while still carrying stale paths, including `8th_Brigade#-28534`, `7th_Brigade#-28752`, `5th_Brigade#-29188`, `4th_Brigade#-29406`, and multiple Union brigades. | #61 command-posture execution and #45 settlement gating now treat `orderState=1 + pathInterrupted=True + movementMode<=0 + activeMove=False` as `stalled-interrupted-order`, even when stale paths remain, and `FallBackToLine` overrides the interrupted recovery path with the doctrine fallback target. Tests cover stale-path retask and fallback override. Fresh restart smoke on the current DLL must confirm `[TacticalCommandPosture]` clears these illegal-idle cases and that decision-matrix rows stop repeating the same stalled `pathInterrupted=True queue=0 activeMove=False` state. |

## Runtime Evidence - 2026-05-11 Hampton / Stalled Interruption Follow-Up

The available Hampton `LogOutput.log` is stale relative to the latest deployed DLL: the log mtime was `2026-05-11 21:52:44 -0500`, while the latest deployed `WhiskeyRealism.dll` is SHA-256 `562a61b5cd0cbbedc6d6002a349cd3d68ebf50ea1d60c941e3a5a9deeaafc57a` (1327104 bytes; 1110 PASS). Treat this section as evidence for vanilla fault classification, not proof that the latest guard failed.

Observed repeated `[TacticalDecisionMatrix]` command rows with `pathInterrupted=True`, `queue=0`, `activeMove=False`, and stale or pending order state while the groups remained in `formation=3:MarchColumn`. Representative rows included:

- `8th_Brigade#-28534 ... paths=2 pathInterrupted=True queue=0 activeMove=False ... orderState=1`
- `7th_Brigade#-28752 ... paths=1 pathInterrupted=True queue=0 activeMove=False`
- `5th_Brigade#-29188 ... paths=1 pathInterrupted=True queue=0 activeMove=False`
- `4th_Brigade#-29406 ... paths=1 pathInterrupted=True queue=0 activeMove=False`
- `3rd_Brigade#-29624 ... paths=1 pathInterrupted=True queue=0 activeMove=False`

This aligns with the vanilla command-interruption branch at decompile lines 117179-117185 and 117188-117205: command groups in contact can be stopped without path cleanup, and vanilla does not clear/restore until close enemy contact disappears. Current Whiskey mitigation is the `stalled-interrupted-order` settlement exception plus posture fallback override; fresh restart smoke is still required before calling the Hampton behavior closed.
...
Focused W&L battle smoke ran with `Enable Tactical Observer = true`, `Enable Tactical Bug Telemetry = true`, `Enable Tactical Fallback Retreat Null Guard = true`, `Enable W&L Tactical Charge Guard = true`, verbose observer logging off, and summary throttle 30 seconds. That smoke ran on DLL SHA-256 `2a7bf702f6408e2a131a43fe6aa1a9ee04b56175ee292375c5167344c29c14c1` (519680 bytes). The current deployed DLL SHA-256 is `562a61b5cd0cbbedc6d6002a349cd3d68ebf50ea1d60c941e3a5a9deeaafc57a` (1327104 bytes; 1110 PASS) and includes #46 objective-chain filtering, config-gated `[TacticalDecisionMatrix]` rows, B6c local reaction/reserve intent runtime wiring, B7+B8 artillery/withdrawal runtime wiring, B5 queued/pending-order settlement gating, tiny-contact weak-point suppression, B5 defensive-hold/brigade-scope discipline, #53 pathfinder discipline, #54 W&L operation null guard, #55 HQ auto-link guard, #56 reserve order-delay guard, #57 reserve-list bias, #58 deployment terrain/facing telemetry, #59 reserve commitment gate, #41 orchestrator charge gate, #60 deployment terrain discipline, #61 operations-ledger posture execution, #62 player-order doctrine, #63 map-knowledge diagnostics, full-spectrum doctrine consumers, and #61 stale active-move hardening. Live B4/B5 smoke emitted 37,250 matrix rows, 19 macro decisions, and 45 group decisions without exception spam, then fixed the observed no-measured-enemy weak-point inflation, matrix W&L/order-friction label inversion, B4 macro force-balance/no-sector inference, B5 probing during vanilla retreat, B5 stance-order stacking while vanilla had queued/pending order state, B5 defensive weak-point/fix probes under defensive macro, and B5 writes to division/army top groups. Fresh runtime proof on the current DLL is still needed for sustained `[TacticalCommandPosture]` objective movement, no stale `activeMove=true paths=0` suppression, `[TacticalOpsLedger]`, `[TacticalCommandAssignment]`, `[TacticalPostureSummary]`, B5 settlement skips, tiny-contact suppression, `defend-hold` / `command-scope` rows, `[TacticalPathfinderDiscipline]`, `[TacticalLocalReaction]`, `[TacticalReserveIntent]`, `[TacticalHqLinkGuard]`, `[TacticalReserveOrderDelayGuard]`, `[TacDeployTerrain]`, `[TacDeployTerrainAdvice]`, `[TacticalOrchestratorChargeGate]`, conditional `[Patch:WLOperationNullGuard]`, risky `[TacticalPathShape]` deltas, `[TacticalIntent]`/`[TacticalPlaybook]`, `[once:b8-microai-check-retreats]`, `[once:b7-cancel-bombard]`, `[TacticalObjectiveGuard]`, and stale-interrupted-order clearing.
