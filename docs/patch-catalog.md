...
| 14 | `RealismCheckboxesLockPatch` | Postfix | `Patches/RealismCheckboxesLockPatch.cs`  | `MainMenu.CheckForCheckBoxUpdates` (193612) | Forces 5 realism CBs ON + frozen: `FogOfWarCB`, `OrderDelaysCB`, `FeudsCB`, `FullReadinessCB`, `AllAutomanageCB`. Belt-and-suspenders writes the underlying `GameVars.usefow=true`, `useorderdelays=true`, `debug_deactivatefeuds=false`, `fullreadiness=false`, and calls `MainMenu.SetAutomanage(true)` directly. |
