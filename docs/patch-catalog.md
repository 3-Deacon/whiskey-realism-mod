...
| 16 | `ArmyGroupManagementPatch`   | Postfix | `Patches/ArmyGroupManagementPatch.cs`   | `AICampaign.CheckArmyGroupManagement` (17705) | v0.2.2 historical army-group steering. After vanilla army-group management, committed top formations sharing a historical operating command are attached to an existing ArmyGroup or used to create one through vanilla `ArmyGroup` APIs; preferred historical commanders are appointed when available. Logs `[once:armygroup]` plus bounded `[Patch:ArmyGroup]` create/attach/appoint lines. |
