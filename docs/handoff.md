...
1. **Battle-history observers in `WarStateObserver`** — track ANV defeats, AoP offensive failures, Burnside's first defeat, Lee invading Pennsylvania, Western theater defeats, Valley operations, war clearly lost. Unlocks the remaining 7 succession events (#1, #3, #4, #5, #6, #11, #12) so they can fire from observed game state without test-mode bypass. **Verified anchors (do NOT use `aifaction[].history` — it doesn't exist):**
    - `BattleMonument.UpdateAllianceWon(...)` at decompile line 76496 — final result hook with `_alliancewon`, `_battleresulttype`, `_battleendtype`, `_casualties`, `_commanderskia0/1`, `_commanderfame`, `_commanderdefamed`, `_battlename`, `_position`. **Postfix this to record battle outcomes into a `BattleHistory` ring buffer on `StrategicCoordinator`.**
    ...
    - From recorded battles, derive: ANV-lost-major-battle (CSA losses where commander.alliance==1 and battleresulttype indicates major defeat), AoP-failed-offensive (Union offensives ending in defeat or stalemate), Burnside-first-defeat (when Burnside is in `commanderskia0/1` or commands losing side), Lee-invading-Pennsylvania (current Lee unit position via `Commander.currentcommand.transform.position` against state boundary).
