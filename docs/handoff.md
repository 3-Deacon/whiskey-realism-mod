...
v0.2.2 shipped (tag + GitHub Release at https://github.com/3-Deacon/whiskey-realism-mod/releases/tag/v0.2.2). Strategic core verified working end-to-end in-game. Current local/deployed main also includes post-release locked-Hard casualty-tolerance integration and #7 role-aware campaign perk steering.

**Post-v0.2.2 backlog**:

1. **Construction/fort doctrine design** — current #20 biases vanilla-valid building candidates but does not deeply score building type + site + theater role. Next building slice should investigate vanilla fort construction and add a `ConstructionIntentLedger` before any direct fort/production patches.
