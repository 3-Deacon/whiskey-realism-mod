...
| **Main branch current** | **Post-v0.2.2 checkpoint.** Built and deployed DLL SHA-256 is `b5f86e5cdb4ef669a25ca6adfb406da343b100344345aa423d2df0e02b31075e`. Adds locked-Hard casualty tolerance, #7 role-aware campaign perk steering, and a hardened #22 W&L command-selection retry on top of the released v0.2.2 smoke-confirmed build. |
...
**W&L command-selection invariant:** vanilla calls `CareerInformationPanel.ShowStartUnitSelectionList()` only once near campaign start, then pauses. `WlCareerStartSelectionRetryPatch` retries that same vanilla popup call while `WlCareerStartGate` says the player has no command, throttled by Unity frame count because the campaign frame can stop advancing while paused. Do not add a `GameVars.frame` lower-bound gate here. `CampaignController.Update` is the primary anchor because it owns the `careerinformationpanel` field; `AICampaign.Update` can fire too early and only logs a fallback no-panel warning. If this regresses, look for `[W&LStartSelection]` lines or one-time `wl-start-selection:*` warnings in `BepInEx/LogOutput.log`.
...
v0.2.2 shipped (tag + GitHub Release at https://github.com/3-Deacon/whiskey-realism-mod/releases/tag/v0.2.2). Strategic core verified working end-to-end in-game. Current pushed/deployed main is `2ccc743` and also includes post-release locked-Hard casualty-tolerance integration and #7 role-aware campaign perk steering. Those two post-release items are built, deployed, hash-verified, and console-tested; they still need an in-game first-fire smoke pass.
...
- **Fresh W&L command-selection timing.** Vanilla only tries the command-selection popup once near campaign start and then pauses. Do not gate retries on `GameVars.frame` at all; use Unity `Time.frameCount` for throttling because the campaign frame can stop before or at the vanilla one-shot frame. Keep #22 anchored on `CampaignController.Update` because `AICampaign.Update` can run before the career panel exists. #22 is intentionally a retry of vanilla `CareerInformationPanel.ShowStartUnitSelectionList(true)`, not a custom command picker.
