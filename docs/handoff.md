...
| **Main branch current** | **v0.2.2 enrichment implemented, pushed, deployed, and smoke-confirmed through #22.** Latest deployed DLL fixes startup/menu lag from hot-path reflection and restores the W&L "choose a unit to command" popup with a bounded retry. Release packaging/tagging is still pending. |
| **Active workstream** | Slice A v0.2.2 release prep: package/tag after one clean runtime smoke pass, then locked-Hard casualty tolerance, deferred #7 concrete steering, and any naval runtime patches proven necessary by smoke. Task 7 offensive-safety Prefix is intentionally deferred until formation directives have more runtime soak. |
...
**W&L command-selection invariant:** vanilla calls `CareerInformationPanel.ShowStartUnitSelectionList()` only once at campaign frame 50, then pauses. `WlCareerStartSelectionRetryPatch` retries that same vanilla popup call while `WlCareerStartGate` says the player has no command, throttled by Unity frame count because the campaign frame can remain at 50 while paused. If this regresses, look for `[W&LStartSelection]` lines or one-time `wl-start-selection:*` warnings in `BepInEx/LogOutput.log`.
...
- **2026-05-04 — latest deployed checkpoint.** Current `origin/main` head is `df5aa28`. Console tests pass; `./build.sh` passes with 0 warnings / 0 errors. Current deployed `dist`/BepInEx DLL SHA-256 is `51a4713a1c1c743d47e86056cce6b91f2d37151f37741baa2e9d008ac092d668`.
...
1. **Release-prep smoke pass on current main** — launch GTCW from a fresh restart, start a W&L campaign, confirm the command-selection popup appears, select a command, advance until weekly AI work fires, then tail `BepInEx/LogOutput.log` for non-spammy first-fire/intervention lines from #8, #18, #19, #20, #21, and #22. Current deployed DLL SHA-256 is `51a4713a1c1c743d47e86056cce6b91f2d37151f37741baa2e9d008ac092d668`.
2. **Concrete steering for `PerkSelectionPatch` (#7)** — still reserved / deferred from v0.2.0. Needs role-aware perk mapping so perks support assigned army/naval roles rather than generic personality flavor. Task 7 offensive-safety Prefix from the formation-directive plan is not #7 and remains deferred until more directive runtime soak.
3. **Full-map runtime coordinate capture** — objective table now covers known W&L eastern/coastal/river IDs. Still capture Vicksburg, Memphis, Baton Rouge, New Orleans, Louisville, St. Louis, Chattanooga, Atlanta, Nashville, and Corinth before relying on western/gulf objective metadata.
...
- **Fresh W&L command-selection timing.** Vanilla only tries the command-selection popup once at campaign frame 50 and then pauses. Do not gate retries on `GameVars.frame > 50`; use Unity `Time.frameCount` for throttling while the campaign frame is stuck at 50. #22 is intentionally a retry of vanilla `CareerInformationPanel.ShowStartUnitSelectionList(true)`, not a custom command picker.
