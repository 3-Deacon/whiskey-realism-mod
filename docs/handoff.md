...
- **2026-05-03 — v0.2.2 historical army-area steering implemented + deployed.** Local implementation adds pure `ArmyAreaDoctrine` / `HistoricalArmyAreaRegistry` / `ArmyAreaLedger`, runtime `ArmyAreaRuntime`, monthly `StrategicCoordinator.ArmyAreas[alliance]`, and #15 `ArmyAreaTheaterPatch` on `AICampaign.UpdateCampaignTheaters`. This uses vanilla's existing `Regiment.theaterposition` + `AICampaign.MoveUnitTo` surfaces to nudge idle AI top formations back toward historical operating areas instead of replacing the campaign army/corps/division system. Logging is bounded: `[ArmyArea]` only on monthly signature change and `[Patch:ArmyArea]` once per return-area correction. Console tests pass; `./build.sh` passes with 0 warnings / 0 errors. DLL deployed to `<GTCW>/BepInEx/plugins/WhiskeyRealism.dll` and verified by SHA-256 (`5c377369bf8a0b03c61c88c1dbd8a823f29ed97815355b88063abe98d6930f59`). Runtime smoke still needs a GTCW restart and a campaign AI tick that hits `UpdateCampaignTheaters`.
