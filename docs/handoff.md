...
| **Main branch current** | **v0.2.2 release checkpoint.** Latest deployed DLL fixes startup/menu lag from hot-path reflection and restores the W&L "choose a unit to command" popup with a bounded retry. |
| **Active workstream** | Post-v0.2.2 planning: deferred #7 concrete steering, deeper construction/fort doctrine, and any naval runtime patches proven necessary by smoke. Task 7 offensive-safety Prefix is intentionally deferred until formation directives have more runtime soak. |
...
v0.2.2 shipped (tag + GitHub Release at https://github.com/3-Deacon/whiskey-realism-mod/releases/tag/v0.2.2). Strategic core verified working end-to-end in-game. Current local/deployed main also includes the post-release locked-Hard casualty-tolerance integration.

**Post-v0.2.2 backlog** (see `docs/patch-catalog.md` §"Pending"):

1. **Concrete steering for `PerkSelectionPatch` (#7)** — still reserved / deferred from v0.2.0. Needs role-aware perk mapping so perks support assigned army/naval roles rather than generic personality flavor. Task 7 offensive-safety Prefix from the formation-directive plan is not #7 and remains deferred until more directive runtime soak.
2. **Construction/fort doctrine design** — current #20 biases vanilla-valid building candidates but does not deeply score building type + site + theater role. Next building slice should investigate vanilla fort construction and add a `ConstructionIntentLedger` before any direct fort/production patches.
