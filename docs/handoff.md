...
| **Active workstream** | Slice A v0.2.2 enrichment smoke/release prep: runtime-smoke #4/#15/#16/#17, finish #7/#8 concrete steering, then integrate policy/recruitment/naval strategy and locked-Hard casualty tolerance |
...
```
StrategicCoordinator (singleton, weekly strategic review + monthly heartbeat)
...
**Read-only mod-state invariant:** Harmony patches READ mod state, never WRITE. State writes happen only on weekly strategic review + event-trigger handlers.
...
2. **Runtime smoke for #17 `ProjectSelectionPatch` plus pending #4/#15/#16 markers** — start/restart GTCW, load or start a W&L campaign, let the campaign map run past frame 50 and at least one AI project update. Expected marker: `[once:project-selection] ProjectSelectionPatch wired (grand-strategy project steering)`. Replacement lines are conditional and only appear when Whiskey changes vanilla's queued project.
3. **Policy/recruitment/naval grand-strategy integration** — design captured at [`docs/superpowers/specs/2026-05-03-grand-strategy-and-research-tree-design.md`](superpowers/specs/2026-05-03-grand-strategy-and-research-tree-design.md). Objective tags and project steering are implemented locally; remaining surfaces are policy picks, recruitment intent, and naval preparation beyond project choice. Verified anchors: `Policies.CheckAIPolicyChange(int alliance)` (policy AI walks ordered `AIPersonality.policies`), `AICampaign.ZoneRecruiting`, `AIArea.GetBestRecruitingState`, `AICampaign.CheckShipConstruction`, `AICampaign.CheckFleetMovements`, `AICampaign.CheckForDefensiveOperations`, `AICampaign.CheckOffensiveMovements`, `AICampaign.CheckTransferOfUnits`, `AICampaign.UpdateCampaignTheaters`, `AICampaign.CheckArmyGroupManagement`, `AIFaction.TransferData`, `ArmyGroup`, `Config/policies.dat`, `Config/projects.dat`, `Config/shiptypes.dat`, and `Campaigns/002/A/aipersonalities.dat`.
4. **Concrete steering for `PerkSelectionPatch` (#7), `RecruitmentPatch` (#8)** — still reserved / deferred from v0.2.0. #8 now has a concrete intended shape: scoped `AIArea.GetBestRecruitingState` Postfix active only during `ZoneRecruiting`, driven by `RecruitmentIntentLedger` so CSA/Union recruitment supports the grand-strategy/front posture without corrupting raid or sea-invasion recruitment. #3 `TransferOfUnitsPatch` is smoke-verified. #4 `DefensiveOpsPatch` is implemented, built, deployed, and hash-verified; runtime first-fire is pending a GTCW restart and a capital-defense tick.
...
6. **Vanilla settings → mod logic integration** — route the locked-Hard `usedcampaignbonus` into `CIC.Effective` to scale `CasualtyTolerance`. Currently the lock is informational only.
7. **`Policy.CurrentChapter` integration** — vanilla's 5-chapter system overlaps with our 4-stage `EraStage`. Map our era stages onto vanilla chapters (or retire `EraStage`). For W&L scenario "002", chapter transitions are: 1 (start), 2 (after 1862-11-05), 3 (after 1864-11-09 if objective 26 accomplished AND 27 not). Decompile reference: `Policy.CheckForChapterUpdate` at line 211604.
