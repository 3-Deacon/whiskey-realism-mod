...
| **Active workstream** | Post-v0.2.2 planning: locked-Hard casualty tolerance, deferred #7 concrete steering, deeper construction/fort doctrine, and any naval runtime patches proven necessary by smoke. Task 7 offensive-safety Prefix is intentionally deferred until formation directives have more runtime soak. |
...
1. **Locked-Hard casualty tolerance integration** — route locked difficulty/bonus into `CIC.Effective` casualty tolerance instead of leaving the lock informational.
...
3. **Construction/fort doctrine design** — current #20 biases vanilla-valid building candidates but does not deeply score building type + site + theater role. Next building slice should investigate vanilla fort construction and add a `ConstructionIntentLedger` before any direct fort/production patches.
4. **Full-map runtime coordinate capture** — objective table now covers known W&L eastern/coastal/river IDs. Still capture Vicksburg, Memphis, Baton Rouge, New Orleans, Louisville, St. Louis, Chattanooga, Atlanta, Nashville, and Corinth before relying on western/gulf objective metadata.
