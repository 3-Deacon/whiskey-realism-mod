...
| **Active workstream** | Slice A v0.2.2 release prep and next enrichment: policy/recruitment/naval strategy, locked-Hard casualty tolerance, then deferred #7/#8 concrete steering. Task 7 offensive-safety Prefix is intentionally deferred until formation directives have more runtime soak. |
...
1. **Policy/naval grand-strategy integration** — design captured at [`docs/superpowers/specs/2026-05-03-grand-strategy-and-research-tree-design.md`](superpowers/specs/2026-05-03-grand-strategy-and-research-tree-design.md). Objective tags, project steering, and recruitment state steering are implemented; remaining surfaces are broader policy timing and naval preparation beyond project choice. Verified anchors: `Policies.CheckAIPolicyChange(int alliance)` (policy AI walks ordered `AIPersonality.policies`), `AICampaign.CheckShipConstruction`, `AICampaign.CheckFleetMovements`, `Config/policies.dat`, `Config/projects.dat`, `Config/shiptypes.dat`, and `Campaigns/002/A/aipersonalities.dat`.
