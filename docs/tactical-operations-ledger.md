...
- **Build/deploy proof:** console harness `769 PASS / 0 FAIL`; `./build.sh` passed with `0 Warning(s)` / `0 Error(s)`; local `dist/WhiskeyRealism.dll` and deployed BepInEx plugin match SHA-256 `51a68c06b92251afdf30e246845bf6f2e4d6058502f2b82e0b3085a19c974cad` (891904 bytes).
- **Runtime smoke:** pending. Current `LogOutput.log` predates the deployed plugin timestamp `2026-05-10 22:27:48 -0500`, so it cannot prove Active operations-ledger runtime behavior.
