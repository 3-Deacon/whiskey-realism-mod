...
- **Build/deploy proof:** console harness `834 PASS / 0 FAIL`; `./build.sh` passed with `0 Warning(s)` / `0 Error(s)`; local `dist/WhiskeyRealism.dll` and deployed BepInEx plugin match SHA-256 `5fdcee189d391502ac85eb107aa894d4d09fe91f5f77fb7fcd3fcbaf775e895b` (931328 bytes).
- **Runtime smoke:** pending. Current `LogOutput.log` mtime is `2026-05-11 21:21:44 -0500`, which predates the deployed plugin timestamp `2026-05-11 21:36:28 -0500`, so it cannot prove Active operations-ledger runtime behavior for this build.
...
Objective anchors come from vanilla battle state before synthetic evidence. The operations ledger now reads `AIBattle.objectivechain[i].objectives` first, then deduplicates any per-group `Regiment.currentsetobjective` anchors. The old no-objective fallback used `enemy-line-{side}` at `(0,0)`, which made #61 reject the target as default/unsafe; the fallback now uses a weighted visible enemy-line centroid only when no map or current objective can be read.
