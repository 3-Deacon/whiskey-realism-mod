...
- **Build/deploy proof:** current player-order worktree console harness `903 PASS / 0 FAIL`; `./build.sh` passed with `0 Warning(s)` / `0 Error(s)`; local `dist/WhiskeyRealism.dll` and deployed BepInEx plugin match SHA-256 `c3c9b7d0f051c24c36fe2d3a7cf3e552bf0df30c945bfa3efb2559332f646ff3` (982528 bytes).
- **Runtime smoke:** pending. Fresh Active operations-ledger smoke is still required after the current deploy.
- **Related W&L player-order surface:** #62 consumes the same tactical orchestrator/operations-ledger intent to produce default-off W&L current-order diagnostics and writes. Current behavior lives in [`docs/wl-player-order-doctrine.md`](wl-player-order-doctrine.md).
...
#62 is a separate default-off W&L player-order writer that reads this doctrine surface but has its own W&L config valve. Keep player-facing current-order behavior in [`docs/wl-player-order-doctrine.md`](wl-player-order-doctrine.md) rather than treating #61 as the owner of W&L order popups.
...
- No unintended player-side or player-subordinate battle-state retasking is observed. Player-facing current-order popups are owned separately by #62 and are validated by the #62 smoke checklist.
...
Current Active smoke boundary: not passed. Fresh operations-ledger, doctrine-consumer, and #62 player-order diagnostics markers are still required after the current deploy.
