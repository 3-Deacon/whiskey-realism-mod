...
- **Build/deploy proof:** console harness `763 PASS / 0 FAIL`; `./build.sh` passed with `0 Warning(s)` / `0 Error(s)`; local `dist/WhiskeyRealism.dll` and deployed BepInEx plugin match SHA-256 `a1f7c1e575fe984477d233c63a98b194c719ee6b28b9172d960661d8ca3944ca` (888832 bytes).
- **Runtime smoke:** pending. Current `LogOutput.log` mtime `2026-05-10 21:30:47 -0500` predates the deployed plugin timestamp `2026-05-10 21:41:01 -0500`, so it cannot prove Active operations-ledger runtime behavior.
