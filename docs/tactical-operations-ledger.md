...
- **Build/deploy proof:** console harness exits 0 across `927` registered tests; `./build.sh` passed with `0 Warning(s)` / `0 Error(s)`; local `dist/WhiskeyRealism.dll` and deployed BepInEx plugin match SHA-256 `51c1ba05d07f730b1a29995820ab0b8bf58c6dbd30a2ae7a655937586ccbde76` (1083904 bytes).
- **Runtime smoke:** pending. The deployed plugin timestamp is `2026-05-15 07:57:38 -0500`; only a fresh `LogOutput.log` after a GTCW restart can prove Active operations-ledger runtime behavior for this build.
...
Current Active smoke boundary: not passed. The deployed plugin timestamp is `2026-05-15 07:57:38 -0500`; only a fresh `LogOutput.log` after restarting GTCW can prove this build. Fresh operations-ledger and doctrine-consumer markers are still required, including `approach-intercept` / `objective-lane` on corridor defense, `scourge-slot-flank-guard` / `scourge-slot-depth` on wide or nested line assembly, the parent command/HQ retained as a reserve anchor in nested single-division battles, and no broad `doctrine=none` fallback for unknown command roles.
