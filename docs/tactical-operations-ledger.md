...
- **Build/deploy proof:** console harness `844 PASS / 0 FAIL`; `./build.sh` passed with `0 Warning(s)` / `0 Error(s)`; local `dist/WhiskeyRealism.dll` and deployed BepInEx plugin match SHA-256 `8eff50e672d70d6a10aa6a8a4c90d37408958026001eba236231e48618eea4dd` (939520 bytes).
- **Runtime smoke:** pending. Current `LogOutput.log` mtime is `2026-05-12 14:35:02 -0500`, which predates the deployed plugin timestamp `2026-05-12 15:00:49 -0500`, so it cannot prove Active operations-ledger runtime behavior for this build.
...
Current Active smoke boundary: not passed. The only current log is stale for this build: `LogOutput.log` mtime `2026-05-12 14:35:02 -0500` predates the deployed plugin timestamp `2026-05-12 15:00:49 -0500`, so fresh operations-ledger and doctrine-consumer markers are still required.
