...
- **Build/deploy proof:** console harness `844 PASS / 0 FAIL`; `./build.sh` passed with `0 Warning(s)` / `0 Error(s)`; local `dist/WhiskeyRealism.dll` and deployed BepInEx plugin match SHA-256 `ed790a0ed8eef492de21b489e920522c0c9605e9484e1937f8390574612f7b9b` (939520 bytes).
- **Runtime smoke:** pending. Current `LogOutput.log` mtime is `2026-05-12 14:34:39 -0500`, which predates the deployed plugin timestamp `2026-05-12 14:44:00 -0500`, so it cannot prove Active operations-ledger runtime behavior for this build.
