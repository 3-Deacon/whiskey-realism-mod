...
Status: implemented, console-tested, built, deployed, hash-verified, and focused-smoke sampled on 2026-05-07; superseded in the current deployed build by B7+B8 wiring DLL `328c74a43f356df4ecb52f38a0df1ec89267eae714fd207af406529b1adffef0` (633856 bytes). Focused W&L battle smoke on the prior telemetry DLL confirmed B2 command/order telemetry and repeated `BUG-TAC-005` objective-chain exposure; user field observation escalated that to #46 objective-chain W&L guard. The current DLL also includes removable `[TacticalDecisionMatrix]` rows plus B7+B8 artillery/withdrawal runtime wiring. The loaded old DLL produced `[TacticalPathShape] reason=backward-first-segment`, so `BUG-TAC-010` is now proof-backed.
