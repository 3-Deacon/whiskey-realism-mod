...
| **O3 — ArmyIntent direct-child enrichment + #42 gate extension** (shipped 2026-05-09; rescoped from the originally planned "Corps echelon" — see [archived rescoped spec](../../plans/archive/2026-05-09-tactical-orchestrator-o3-armyintent-direct-child-enrichment.md) and [archived design](archive/2026-05-09-tactical-orchestrator-o3-corps-design.md)) | [`docs/superpowers/plans/archive/2026-05-09-tactical-orchestrator-o3-armyintent-direct-child-enrichment.md`](../../plans/archive/2026-05-09-tactical-orchestrator-o3-armyintent-direct-child-enrichment.md) | Additive `ArmyOrchestrator.ArmyIntent.DirectChildIntents` (no separate `CorpsOrchestrator` echelon — adversarial review found vanilla has no corps tier, `commandhierarchyshift` makes hierarchy depth era-dependent and the top group can be Army/Corps/Division). New types: `DirectChildIntent`, `TacticalDirectChildAllocator` (Main / SupportMain / RefuseRight / RefuseLeft / Screen / Reserve / Fallback / Fix role assignment), `TacticalDirectChildGate`, `DirectChildSnapshot`, `DirectChildEvidence`, `DirectChildRole`, `DirectChildAxis`, `DirectChildGateDecision`. Per-side direct-child discovery walks `BattleUnits.completeunitlist` filtered by alliance. #42 `BattleFeudActionGatePatch` extended with `TacticalDirectChildGate.Decide` consultation between the W&L decision and `bunits.SetWaypoint` (Main/SupportMain compares `IntendedTargetSector` against orchestrator `AxisSector` OR child `PrimarySector`; Reserve always denies; Fallback denies toward-enemy bearings; Fix bounds distance; Screen/Refuse require in-sector target). | `Enable Tactical Orchestrator Direct-Child Gate = false` (default OFF — conservative because the gate writes feud-movement decisions). Gate-OFF run is a pure no-op against vanilla feud movement. | Gate-OFF + gate-ON smoke verified on `orch/o2-intent`: per-side `[TacticalDirectChildDiscovery]` with distinct parent-army IDs, distinct child sectors, mix of synth-army + real brigade children, differentiated allocator roles, zero `[TacticalDirectChildGate] action=deny`, zero exceptions, zero missing-anchor warnings. Deny path harness-verified in 9 Task 8 unit tests + 4 `ParseInstanceIdFromChildId` regression cases. Final DLL `2a9e1e2f3b4bb547254fe69d02c355b69f65f02016875cd4ca298c90a03237b0`; harness 661 PASS / 0 FAIL. O4/O5/O6/O7 defer boundaries from the rescoped spec hold unchanged. |
