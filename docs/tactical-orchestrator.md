...
- **Current tactical orchestrator state:** O0/O1/O2/O3, #58 deployment observer, Slice 0 command-node tree, Slice 1 reserve commitment gate, Slice 3 #41 charge gate, #60 deployment terrain/facing discipline, #61 operations-ledger posture executor, the full-spectrum doctrine consumers for stance/charge/reserve/fallback/artillery, and #62 default-off W&L player-order doctrine are implemented in the current player-order worktree.
- **Current verification:** console harness `898 PASS / 0 FAIL`; `./build.sh` passed with `0 Warning(s)` / `0 Error(s)`; local `dist/WhiskeyRealism.dll` and deployed BepInEx plugin match SHA-256 `5398831614e855f90b156c734fee50348d0bb6b10c8f0390b7a3d2499ba57740` (977920 bytes).
- **Runtime smoke boundary:** Active full-spectrum doctrine smoke and #62 player-order diagnostics/write smoke are pending after the current deploy. A fresh battle log must prove `[TacticalCommanderMode] mode=Active`, `[TacticalOpsLedger]`, `[TacticalCommandAssignment]`, `[TacticalCommandPosture]`, `[TacticalPostureSummary]`, `[TacticalDoctrineCharge]`, `[TacticalOrchestratorChargeGate]`, `[TacticalGroupDecision]`, bounded reserve/fallback lines, bounded `[PlayerOrderIntent]` diagnostics, and no repeated errors before the doctrine system is release-smoke-verified.
...
| #61 operations-ledger posture executor + doctrine consumers | Shipped on `main` | Harness/build/deploy/hash verified; Active smoke pending fresh battle log |
| #62 W&L player-order doctrine | Implemented in current player-order worktree | Harness/build/deploy/hash verified at `5398831614e855f90b156c734fee50348d0bb6b10c8f0390b7a3d2499ba57740`; diagnostics/write smoke pending fresh W&L player-subordinate battle |
...
| W&L player-facing current orders | #62 `PlayerSubordinateOrderPatch` | Tactical orchestrator to vanilla W&L current-order bridge for player-subordinate commands | `Enable W&L Player Order Doctrine`, diagnostics via `Enable W&L Player Order Doctrine Diagnostics`, plus `Tactical Commander Mode = Active` for writes |
...
Living references: [`docs/tactical-operations-ledger.md`](tactical-operations-ledger.md) for #61 operations-ledger behavior and [`docs/wl-player-order-doctrine.md`](wl-player-order-doctrine.md) for #62 player-facing W&L current orders.
...
1. Run Active full-spectrum doctrine battle smoke on deployed DLL `5398831614e855f90b156c734fee50348d0bb6b10c8f0390b7a3d2499ba57740` and prove `[TacticalCommanderMode] mode=Active`, `[TacticalOpsLedger]`, `[TacticalCommandAssignment]`, `[TacticalCommandPosture]`, `[TacticalPostureSummary]`, `[TacticalGroupDecision]`, `[TacticalDoctrineCharge]`, `[TacticalOrchestratorChargeGate]`, bounded reserve/fallback rows, and no repeated errors.
2. Run #62 diagnostics-only W&L player-order smoke, then focused write-enabled smoke in a player-subordinate battle. Prove bounded `[PlayerOrderIntent]` rows, no writes while disabled, accepted vanilla current-order session advance when enabled, no player-CIC/non-W&L writes, and no stale retry spam.
3. Run Slice 1, Slice 3, and #60 focused battle smoke on the latest deployed tactical DLL.
4. If smoke passes, archive the completed operations-ledger and player-order doctrine specs/plans, then update this file plus `docs/handoff.md`, `docs/patch-catalog.md`, and `MEMORY.md` with runtime proof.
...
- W&L player-order living reference: `docs/wl-player-order-doctrine.md`
...
- W&L player-order execution plan: `docs/superpowers/plans/2026-05-12-wl-player-order-doctrine-implementation.md` (point-in-time traceability; current behavior lives in `docs/wl-player-order-doctrine.md`)
