...
- **Current verification:** console harness `769 PASS / 0 FAIL`; `./build.sh` passed with `0 Warning(s)` / `0 Error(s)`; local `dist/WhiskeyRealism.dll` and deployed BepInEx plugin match SHA-256 `9c5217a9430a0f03bf2cbbae68328719324801a3e0d9099bf039bf69f95b6b6f` (891904 bytes).
- **Runtime smoke boundary:** Active operations-ledger smoke is pending because current `LogOutput.log` mtime `2026-05-10 22:13:39 -0500` predates the deployed plugin timestamp `2026-05-10 22:15:50 -0500`. A fresh battle log must prove `[TacticalOpsLedger]`, `[TacticalCommandAssignment]`, `[TacticalCommandPosture]`, and `[TacticalPostureSummary]` without repeated errors before #61 is release-smoke-verified.
- **Latest log-driven fix:** the observed `1st Brigade` / `38th New York` courier case showed command nodes keyed by `Regiment.gameObject.GetInstanceID()` while tactical consumers resolved by the `Regiment` component id, so #61 could wire but not attach to the live ledger rows. Command intent and ledger lookup now prefer GameObject id and fall back to component id across #61, #41, #57, #59, B8 fallback observation, #35 monitor lookup, and #45 stance lookup. Hampton's Legion / 8th Brigade exposed a second allocation defect: a fixing-sector assignment could outrank severe local overmatch, leaving an isolated brigade fighting a superior force instead of falling back. Severe overmatch now produces a `Fallback` direct-child role before `Fix`. The 1st/3rd Brigade facing trace then exposed a #61 execution defect: command groups could report `groupformation=Line` while the visible `formation` remained `MarchColumn`, causing defensive/fallback correction to stall behind recent-order cooldowns; defensive/fallback corrections now compare visible, ordered, and group formation, retry urgent visible-mismatch formation corrections after 5 seconds, and pass a manual threat-facing rotation for close formation refreshes. The Hampton flank cluster exposed a separate #61 task-boundary issue: a close-engaged attacking command with `flanksthreated` / `outflanked` evidence stayed in `AttackObjective` and was blocked by courier `order-pending`. Close local flank emergencies now temporarily execute as `GuardFlank` current-position formation/facing corrections and may bypass pending courier state only for that non-moving formation correction. The earlier `1st_Brigade#-27662` interrupted-idle fix remains in place: non-reserve hold/fallback tasks with interrupted empty paths are illegal idle and can recover through bounded `RecoveryPath` waypoints.
...
| #61 operations-ledger posture executor | Shipped on `main` | Harness/build/deploy/hash verified at `9c5217a9430a0f03bf2cbbae68328719324801a3e0d9099bf039bf69f95b6b6f`; Active smoke pending fresh battle log |
...
1. Run Active operations-ledger battle smoke on deployed DLL `9c5217a9430a0f03bf2cbbae68328719324801a3e0d9099bf039bf69f95b6b6f` and prove `[TacticalOpsLedger]`, `[TacticalCommandAssignment]`, `[TacticalCommandPosture]`, `[TacticalPostureSummary]`, no repeated errors, and no player-subordinate retasking.
