...
- **Current verification:** console harness `771 PASS / 0 FAIL`; `./build.sh` passed with `0 Warning(s)` / `0 Error(s)`; local `dist/WhiskeyRealism.dll` and deployed BepInEx plugin match SHA-256 `f5af359f71ed0a56f2ee2856d8634ea9913ff8bf96c768913216eb5fa7282ec8` (891904 bytes).
- **Runtime smoke boundary:** Active operations-ledger smoke is pending because current `LogOutput.log` predates the deployed plugin timestamp `2026-05-10 22:37:49 -0500`. A fresh battle log must prove `[TacticalOpsLedger]`, `[TacticalCommandAssignment]`, `[TacticalCommandPosture]`, and `[TacticalPostureSummary]` without repeated errors before #61 is release-smoke-verified.
...
| #61 operations-ledger posture executor | Shipped on `main` | Harness/build/deploy/hash verified at `f5af359f71ed0a56f2ee2856d8634ea9913ff8bf96c768913216eb5fa7282ec8`; Active smoke pending fresh battle log |
...
1. Run Active operations-ledger battle smoke on deployed DLL `f5af359f71ed0a56f2ee2856d8634ea9913ff8bf96c768913216eb5fa7282ec8` and prove `[TacticalOpsLedger]`, `[TacticalCommandAssignment]`, `[TacticalCommandPosture]`, `[TacticalPostureSummary]`, no repeated errors, and no player-subordinate retasking.
